...
frappe.query_reports["BOM RM STATUS"] = {
	"filters": [{
            "fieldname": "company",
            "label": __("Company"),
            "fieldtype": "Link",
        ...
        },
	{
		...
		frappe.call({
			method: "shark.shark.report.bom_rm_status.bom_rm_status.fetch_project_details",
			args: {
         	 		"project":project
        		},
			async: false,
	...
	onload: function(report) {
        ...
        report.page.add_inner_button(__("Make Issue"),
            function() {
                var reporter = frappe.query_reports["BOM RM STATUS"];
		...
		for (var i = 0; i < reportData.length; i++) {
			...
			if (qty > 0) {
				table_data_list.push({"item_code":item_code,
			    	...
			    	frappe.call({
        			method:"shark.shark.report.bom_rm_status.bom_rm_status.make_issue",
        			args: {
...
function getReportData() {
    var reportData = [];
    frappe.call({
        method: "shark.shark.report.bom_rm_status.bom_rm_status.get_report_data",
        async: false,
        callback: function(r) {
...
function check_for_whole_number(bomNO, qty, query_report) {
    ...
    frappe.call({
        method: "shark.shark.report.bom_rm_status.bom_rm_status.check_for_whole_number",
        args: {
            "bomno": bomNO
        },
        async: false,
