...
def create_selected_row_po(checked_rows,supplier):
	...
	for items_details in items:
		...
		project = frappe.db.sql("""select project from `tabMaterial Request` 
		where name='"""+items_details['material_request_no']+"""'""", as_dict=1)
...
def create_po(material_request):
	...
	for supplier_details in filtered_list:
		...
		mri.name as material_request_item,mr.project,mri.warehouse,mr.schedule_date as reqd_by_date,mri.stock_uom,id.default_supplier from `tabMaterial Request` as mr inner join 
