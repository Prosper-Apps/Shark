...
cur_frm.cscript['Make BOM'] = function(cdt, cdn) {
    ...
    frappe.model.open_mapped_doc({
        method: "shark.api.make_bom_for_boq_lite",
        frm: cur_frm
    })
}


frappe.ui.form.on("BOQ Light", "refresh", function(frm) {
    cur_frm.set_query("stock_uom", "items", function(frm, cdt, cdn) {
        var d = locals[cdt][cdn];
        var uom_list = [];
        console.log("Item Code:: " + d.item_code);
        frappe.call({
            method: "shark.api.get_stock_uom",
            args: {
...
var parentList = [];
frappe.call({
        method: "shark.shark.doctype.boq_light.boq_light.fetch_parent_list",
        args: {
            "parent": boq_lite,
        },
        async: false,
...
function update_boq_lite_item(item_code, name, is_raw_material) {
    frappe.call({
        method: "shark.api.update_boq_lite_item",
        args: {
            "item_code": item_code,
            "name": name,
...
frappe.ui.form.on("BOQ Light Item", "immediate_parent_item", function(frm, doctype, name) {
